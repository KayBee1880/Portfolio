...
export default function ExperiencePage() {
    ...
    const [active, setActive] = useState<ExperienceType | "All">("All");
    const [q, setQ] = useState("");

    const filtered = useMemo(() => {
        const query = q.trim().toLowerCase();
        return experience.filter((e) => {
            const matchesType = active === "All" || e.type === active;

            const blob =
                `${e.company} ${e.role} ${e.dates} ${e.location} ${e.type} ` +
                `${(e.tags ?? []).join(" ")} ${(e.bullets ?? []).join(" ")}`;

            const matchesQuery = !query || blob.toLowerCase().includes(query);
            ...
            <main className="mx-auto max-w-6xl px-6 pb-20 pt-10">
                ...
                <div className="mb-10">
                    ...
                    <p className="mt-3 max-w-3xl text-zinc-600 dark:text-white/70">
                        Featured roles plus programs, apprenticeships, and leadership work
                        across software, cloud, and security.
                    </p>
                    ...
                    <div className="mt-6 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                        ...
                        <div className="flex flex-wrap gap-2">
                            ...
                            {experienceTypes.map((t) => (
                                <button
                                    key={t}
                                    onClick={() => setActive(t)}
                                    className={`rounded-full border px-4 py-2 text-sm font-semibold transition ${active === t
                                            ? "border-zinc-900 bg-zinc-900 text-white dark:border-white/20 dark:bg-white dark:text-black"
                                            : "border-zinc-200 bg-white text-zinc-900 hover:bg-zinc-50 dark:border-white/10 dark:bg-white/5 dark:text-white dark:hover:bg-white/10"
                                        }`}
                                >
                        ...
                        <input
                            ...
                            placeholder="Search experience (e.g., AWS, Splunk, Java, AD)…"
                ...
                <div className="grid gap-6">
                    {filtered.length === 0 ? (
                        ...
                        filtered.map((e) => <ExperienceCard key={e.id} item={e} />)
                    )}
                </div>
            </main>
        </div>
